...
export const Footer = forwardRef<HTMLElement, FooterProps>(
  (
    ...
    const variantClasses = {
      ...
      colored: color === 'primary' ? 'bg-primary-600 text-white' : 'bg-secondary-600 text-white',
    };
      ...
      <footer ref={ref} className={classes} {...rest}>
        <div className="flex flex-col md:flex-row items-center justify-between">
          ...
          <div className="flex items-center mb-4 md:mb-0">
            ...
            {copyright && <div className="text-sm">{copyright}</div>}
          </div>

          {/* Kinder-Elemente (in der Mitte) */}
          {children && <div className="mb-4 md:mb-0 md:mx-4">{children}</div>}

          {/* Links (rechts) */}
          {links && <div className="flex items-center space-x-4 text-sm">{links}</div>}
        </div>
      </footer>
    );
  }
