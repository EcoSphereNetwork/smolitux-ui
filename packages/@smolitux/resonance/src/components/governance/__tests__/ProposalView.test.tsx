...
describe('ProposalView', () => {
  const mockProposal = {
    ...
    description:
      "Proposal to implement a token burning mechanism to reduce supply and increase value. This will help maintain the token's value over time and reward long-term holders.",
    status: 'active',
  ...
  it('renders correctly with proposal data', () => {
    ...
    expect(
      screen.getByText(/proposal to implement a token burning mechanism/i)
    ).toBeInTheDocument();
  ...
  it('displays comments', () => {
    ...
    expect(
      screen.getByText('I support this proposal as it will help maintain token value.')
    ).toBeInTheDocument();
    expect(
      screen.getByText("I'm concerned about the impact on transaction volume.")
    ).toBeInTheDocument();
  ...
  it('disables voting when user has no voting power', () => {
    render(<ProposalView proposal={mockProposal} userVotingPower={0} onVote={mockOnVote} />);
  ...
  it('allows commenting on the proposal', () => {
    render(<ProposalView proposal={mockProposal} onComment={mockOnComment} />);
  ...
  it('calls onBack when back button is clicked', () => {
    render(<ProposalView proposal={mockProposal} onBack={mockOnBack} />);
  ...
  it('calls onShare when share button is clicked', () => {
    render(<ProposalView proposal={mockProposal} onShare={mockOnShare} />);
