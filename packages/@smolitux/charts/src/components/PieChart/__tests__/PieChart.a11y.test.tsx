...
describe('PieChart Accessibility', () => {
  ...
  test('should include descriptive title', () => {
    const { container } = render(<PieChart data={mockData} title="Distribution Chart" />);
  ...
  test('should provide accessible color contrast', () => {
    ...
    const blackText = Array.from(valueTexts).find(
  ...
  test('should provide alternative text description for screen readers', () => {
    ...
    expect(svg).toHaveAttribute(
      'aria-label',
      'Distribution chart showing Slice 1 at 35%, Slice 2 at 25%, Slice 3 at 20%, Slice 4 at 15%, and Slice 5 at 5%'
    );
  });

  test('should have accessible legend with sufficient contrast', () => {
    const { container } = render(<PieChart data={mockData} legendTextColor="#000000" />);
