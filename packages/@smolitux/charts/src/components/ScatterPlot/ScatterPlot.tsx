...
export const ScatterPlot = forwardRef<SVGSVGElement, ScatterPlotProps>(
  (
    ...
    const { xMin, xMax, yMin, yMax, normalizedSeries, regressionLines } = useMemo(() => {
    ...
    const renderPoint = (
      ...
      const handleMouseEnter = () => {
        if (showTooltips) {
          setHoveredPoint({
            point,
            seriesIndex,
            x: x + (typeof size === 'number' ? size : 0) / 2,
            y: y - size,
          });
        }
      };
      ...
      switch (shape) {
        case 'square':
          return (
            <rect
              ...
              x={x - (typeof size === 'number' ? size : 0) / 2}
              y={y - (typeof size === 'number' ? size : 0) / 2}
              width={size}
              height={size}
        ...
        case 'triangle':
          const triangleSize = typeof size === 'number' ? size : 0;
        ...
        case 'diamond':
          const diamondSize = typeof size === 'number' ? size : 0;
          const diamondPath = `M ${x} ${y - diamondSize / 2} L ${x + diamondSize / 2} ${y} L ${x} ${y + diamondSize / 2} L ${x - diamondSize / 2} ${y} Z`;
          return (
            <path
              key={`point-${seriesIndex}-${point.x}-${point.y}`}
              d={diamondPath}
        ...
        case 'cross':
          const crossSize = typeof size === 'number' ? size / 2 : 0;
          return (
            <g
              key={`point-${seriesIndex}-${point.x}-${point.y}`}
              className={`transition-opacity ${animated ? 'animate-fade-in' : ''}`}
          ...
          <g className="grid-lines">
            ...
            {xTicks.map((tick) => (
              <line
                ...
                x1={paddingLeft + tick.position * Number(drawingWidth)}
                y1={paddingTop}
                x2={paddingLeft + tick.position * Number(drawingWidth)}
        ...
        {showRegressionLine && (
          <g className="regression-lines">
            {regressionLines.map((line) => {
              const x1 =
                paddingLeft + ((line.points[0][0] - xMin) / (xMax - xMin)) * Number(drawingWidth);
              const y1 =
                paddingTop + (1 - (line.points[0][1] - yMin) / (yMax - yMin)) * drawingHeight;
              const x2 =
                paddingLeft + ((line.points[1][0] - xMin) / (xMax - xMin)) * Number(drawingWidth);
              const y2 =
                paddingTop + (1 - (line.points[1][1] - yMin) / (yMax - yMin)) * drawingHeight;
        ...
        <g className="x-axis">
          ...
          {xTicks.map((tick) => (
            <g key={`x-tick-${tick.value}`}>
              ...
              />
              <text
                x={paddingLeft + tick.position * Number(drawingWidth)}
