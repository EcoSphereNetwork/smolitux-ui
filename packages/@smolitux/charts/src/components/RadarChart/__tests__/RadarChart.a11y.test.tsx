...
describe('RadarChart Accessibility', () => {
  ...
  test('should have appropriate ARIA attributes', () => {
    ...
    expect(svg).toHaveAttribute(
      'aria-label',
      'Radar chart showing skills assessment across 6 technologies'
    );
  });

  test('should include descriptive title', () => {
    const { container } = render(<RadarChart data={mockData} title="Skills Radar Chart" />);
  ...
  test('should provide alternative text description for screen readers', () => {
    ...
    expect(svg).toHaveAttribute(
      'aria-label',
      'Radar chart showing JavaScript: 80%, React: 90%, CSS: 70%, HTML: 95%, Node.js: 75%, TypeScript: 85%'
    );
  });
  ...
  test('should have accessible legend with sufficient contrast', () => {
    ...
    const { container } = render(
      <RadarChart data={multiSeriesData} showLegend={true} legendTextColor="#000000" />
    );
  ...
  test('should have accessible level indicators', () => {
    const { container } = render(
      <RadarChart data={mockData} levels={5} levelLabelColor="#000000" />
    );
    ...
    const blackLabel = Array.from(levelLabels).find(
      (label) => label.getAttribute('fill') === '#000000'
    );
  ...
  test('should have accessible value labels when shown', () => {
    const { container } = render(
      <RadarChart data={mockData} showValues={true} valueTextColor="#000000" />
    );
    ...
    const blackLabel = Array.from(valueLabels).find(
      (label) => label.getAttribute('fill') === '#000000'
    );
