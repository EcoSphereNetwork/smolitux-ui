...
export const RadarChart = forwardRef<SVGSVGElement, RadarChartProps>(
  (
    ...
    const { processedData, axes, allValues, actualMaxValue } = useMemo(() => {
      // Alle eindeutigen Achsen extrahieren
      const allAxes = Array.from(
        new Set(data.flatMap((series) => series.values.map((point) => point.axis)))
      );
                    ...
                    {series.values.map((point, pointIndex) => {
                      ...
                      return (
                        <circle
                          ...
                          onMouseEnter={() =>
                            handlePointMouseEnter(point, seriesIndex, coord.x, coord.y)
                          }
