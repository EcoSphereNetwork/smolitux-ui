...
import { useTheme, defaultTheme } from '@smolitux/theme';
import type { Theme, ColorShades } from '@smolitux/theme';
  ...
  (
    ...
    const defaultColors = useMemo(() => {
      if (
        ...
        const scale = theme.colors[colorScheme] as ColorShades;
        return [
          scale[500],
          scale[600],
          scale[400],
    ...
    // Datenaufbereitung
    const { allCategories, yMin, yMax, normData } = useMemo(() => {
      // Alle Kategorien/Labels aus allen Serien extrahieren
      const categories = Array.from(
        new Set(seriesArray.flatMap((series) => series.data.map((d) => d.label)))
      );
      ...
      return (
        <g>
          {allCategories.map((category, categoryIndex) => {
            const categoryX =
              paddingLeft + (Number(drawingWidth) / allCategories.length) * (categoryIndex + 0.5);
                    ...
                    return (
                      <g key={`bar-${series.id}-${category}`}>
                        <rect
                          ...
                          strokeWidth={1}
                          stroke={
                            themeMode === 'dark' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.5)'
                          }
                          rx={2}
              ...
              return (
                <g key={`category-${category}`}>
                  {normData.map((series, seriesIndex) => {
                    ...
                    const groupOffsetX =
                      (seriesIndex - seriesArray.length / 2 + 0.5) * (barWidth + barSpacing);
    ...
    const renderHorizontalBars = () => {
      ...
      return (
        <g>
          {allCategories.map((category, categoryIndex) => {
            const categoryY =
              paddingTop + (drawingHeight / allCategories.length) * (categoryIndex + 0.5);
              ...
              return (
                <g key={`category-${category}`}>
                  {normData.map((series, seriesIndex) => {
                    ...
                    // Position innerhalb der Gruppe berechnen
                    const groupOffsetY =
                      (seriesIndex - seriesArray.length / 2 + 0.5) * (barHeight + barSpacing);
