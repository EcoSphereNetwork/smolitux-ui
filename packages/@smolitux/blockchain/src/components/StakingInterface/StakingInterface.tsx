...
export const StakingInterface: React.FC<StakingInterfaceProps> = ({
  ...
  const totalEarned = userStakes.reduce(
    (total, stake) => total + parseFloat(stake.earned),
    0
  ).toFixed(6);
  ...
  const handleClaimRewards = async (stakeId: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Abheben der Belohnungen fehlgeschlagen.';
      console.error(errorMessage);
    } finally {
      setIsClaiming(false);
    }
  ...
  const handleClaimAllRewards = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Abheben aller Belohnungen fehlgeschlagen.';
      console.error(errorMessage);
    } finally {
      setIsClaiming(false);
    }
        ...
        <div>
          ...
          {selectedPoolId && (
            <Card className="mb-6 p-4">
              ...
              <div className="mb-4">
                <label htmlFor="stakeAmount" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Betrag
                </label>
              ...
              <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                ...
                {selectedPool?.earlyWithdrawalFee !== undefined && (
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-gray-500 dark:text-gray-400">Frühzeitige Abhebungsgebühr:</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
            ...
            {loading ? (
              ...
              pools.map(pool => (
                <Card key={pool.id} className={`p-4 ${!pool.isActive ? 'opacity-60' : ''}`}>
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between mb-2">
                      <span className="text-sm text-gray-500 dark:text-gray-400">Gesamt gestaked:</span>
                      <span className="text-sm font-medium text-gray-900 dark:text-white">
          ...
          <Card className="mb-6 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                ...
                <p className="text-xl font-semibold text-gray-900 dark:text-white">
                  {userStakes.reduce(
                    (total, stake) => total + parseFloat(stake.amount),
                    0
                  ...
                  <Card key={stake.id} className="p-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-2">
                      ...
                      <Button
                        ...
                        disabled={isUnstaking || (!stake.isUnlocked && stake.unlockAt && new Date() < stake.unlockAt)}
                      >
                        {isUnstaking
                          ? 'Wird unstaked...'
