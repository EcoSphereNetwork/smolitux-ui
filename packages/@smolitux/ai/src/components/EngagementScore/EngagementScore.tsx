              ...
              <div className="md:w-1/3">
                <div className="p-4 bg-white dark:bg-gray-700 rounded-lg shadow-sm">
                  ...
                  <div className="text-center text-xs text-gray-500 dark:text-gray-400">
                    {analysisTimestamp && (
                      <p>Analysiert am {formatDate(analysisTimestamp)}</p>
                    )}
                  </div>
                </div>
              </div>
                      ...
                      <div className="relative pt-1">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center">
                            <div className={`w-3 h-3 rounded-full ${getScoreBackgroundColor(score)} mr-1`} />
                  ...
                  {showExplanation && (
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Der Engagement-Score misst, wie gut Ihr Inhalt bei der Zielgruppe ankommt und Interaktionen auslöst.
                        Ein höherer Score bedeutet, dass Ihr Inhalt mehr Engagement erzeugt als vergleichbare Inhalte.
                      </p>
                      
                      {score >= 80 && (
                        <p className="text-sm text-green-600 dark:text-green-400 mt-2">
                          Ihr Inhalt erzielt ein ausgezeichnetes Engagement und übertrifft die meisten vergleichbaren Inhalte.
                        </p>
                      )}
                      
                      {score >= 60 && score < 80 && (
                        <p className="text-sm text-blue-600 dark:text-blue-400 mt-2">
                          Ihr Inhalt erzielt ein gutes Engagement und übertrifft viele vergleichbare Inhalte.
                        </p>
                      )}
                      
                      {score >= 40 && score < 60 && (
                        <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-2">
                          Ihr Inhalt erzielt ein durchschnittliches Engagement, vergleichbar mit ähnlichen Inhalten.
                        </p>
                      )}
                      
                      {score >= 20 && score < 40 && (
                        <p className="text-sm text-orange-600 dark:text-orange-400 mt-2">
                          Ihr Inhalt erzielt ein unterdurchschnittliches Engagement und könnte verbessert werden.
                        </p>
                      )}
                      
                      {score < 20 && (
                        <p className="text-sm text-red-600 dark:text-red-400 mt-2">
                          Ihr Inhalt erzielt ein niedriges Engagement und sollte überarbeitet werden.
                        </p>
                      )}
                    </div>
                  )}
                    ...
                    <div className="relative pt-1">
                      <div className="h-2 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                        {metric.benchmark && (
                          <div
                            className="absolute h-4 w-1 bg-gray-400 dark:bg-gray-500 rounded-full transform -translate-y-1"
                            style={{ left: `${(metric.benchmark / (metric.benchmark * 2)) * 100}%` }}
                          />
                        )}
                        ...
                        <div
                          className={`h-2 rounded-full ${getScoreBackgroundColor(
                            metric.benchmark
                              ? (metric.value / metric.benchmark) * 50 + 50
                              : 50
                          )}`}
                          style={{
                            width: `${
