...
export const FederatedSearch: React.FC<FederatedSearchProps> = ({
  ...
  const selectTrustedPlatforms = () => {
    const trustedPlatformIds = platforms
      .filter(p => p.isTrusted)
      .map(p => p.id);
    ...
    <div className={className}>
      ...
      {results.length > 0 ? (
        ...
        <div className="py-12 text-center text-gray-500 dark:text-gray-400">
          ...
          <p className="mt-2">Versuchen Sie, Ihre Suchbegriffe zu ändern oder andere Plattformen auszuwählen.</p>
        </div>
      ) : null}
    </div>
  );
