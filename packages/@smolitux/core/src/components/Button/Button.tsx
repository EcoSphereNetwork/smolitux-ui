...
export const Button = memo(forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  const stateClasses = {
    ...
    disabled: disabled || loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer',
    hover: hoverable && !disabled && !loading ? 'hover:opacity-90' : '',
    focus: focusable && !disabled && !loading ? 'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500' : '',
    pressed: isPressed && !disabled && !loading ? 'transform scale-95' : '',
  ...
  const buttonClasses = [
    ...
    variant !== 'unstyled' ? shapeClasses[shape] : '',
    fullWidth ? 'w-full' : '',
    variantClasses[variant],
    variant !== 'link' && variant !== 'unstyled' ? sizeClasses[size] : '',
    stateClasses.active,
