...
describe('A11yButton', () => {
  ...
  test('sets aria-label for icon buttons', () => {
    render(
      <A11yButton isIconButton label="Menu Button" leftIcon={<span data-testid="icon">🔍</span>} />
    );
  ...
  test('creates hidden description element when description is provided', () => {
    render(<A11yButton description="This action cannot be undone">Delete</A11yButton>);
  ...
  test('does not call onClick when disabled', async () => {
    const handleClick = jest.fn();
    render(
      <A11yButton isDisabled onClick={handleClick}>
        Click Me
      </A11yButton>
    );
