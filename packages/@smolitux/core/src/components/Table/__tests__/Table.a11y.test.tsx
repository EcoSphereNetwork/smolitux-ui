...
describe('Table Accessibility', () => {
  ...
  it('should not have accessibility violations in basic state', async () => {
    const { violations } = await a11y.testA11y(<Table columns={columns} data={data} />);
  ...
  it('should have correct ARIA attributes for sortable columns', () => {
    ...
    expect(nameHeader).toHaveAttribute(
      'aria-label',
      expect.stringContaining('Name, klicken zum Sortieren')
    );
    expect(nameHeader).toHaveAttribute('tabIndex', '0');

    expect(ageHeader).toHaveAttribute('role', 'columnheader');
    expect(ageHeader).toHaveAttribute(
      'aria-label',
      expect.stringContaining('Alter, klicken zum Sortieren')
    );
  ...
  it('should update aria-sort attribute when sorting', () => {
    ...
    expect(nameHeader).toHaveAttribute(
      'aria-label',
      expect.stringContaining('sortiert aufsteigend')
    );
    ...
    expect(nameHeader).toHaveAttribute(
      'aria-label',
      expect.stringContaining('sortiert absteigend')
    );
  });
  ...
  it('should have accessible pagination controls', () => {
    render(<Table columns={columns} data={data} paginated={true} itemsPerPage={1} />);
  ...
  it('should have accessible loading and empty states', () => {
    ...
    // Check loading state
    rerender(<Table columns={columns} data={[]} loading={true} />);
