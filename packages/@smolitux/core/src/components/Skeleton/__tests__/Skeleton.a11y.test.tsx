...
describe('Skeleton Accessibility', () => {
  ...
  it('should have no accessibility violations with standard Skeleton', async () => {
    const { container } = render(<Skeleton aria-label="Lädt Text" width={200} height={20} />);
  ...
  it('should have no accessibility violations with A11y Skeleton', async () => {
    const { container } = render(<Skeleton.A11y ariaLabel="Lädt Text" width={200} height={20} />);
  ...
  it('should handle different variants correctly', () => {
    const { rerender } = render(<Skeleton.A11y variant="text" ariaLabel="Lädt Text" />);

    let skeletonItem = screen.getByRole('status').querySelector('div');
    expect(skeletonItem).toHaveClass('rounded');

    rerender(<Skeleton.A11y variant="circular" ariaLabel="Lädt Avatar" />);
  ...
  it('should handle different animations correctly', () => {
    const { rerender } = render(<Skeleton.A11y animation="pulse" ariaLabel="Lädt Text" />);

    let skeletonItem = screen.getByRole('status').querySelector('div');
    expect(skeletonItem).toHaveClass('animate-pulse');

    rerender(<Skeleton.A11y animation="wave" ariaLabel="Lädt Text" />);

    skeletonItem = screen.getByRole('status').querySelector('div');
    expect(skeletonItem).toHaveClass('animate-skeleton-wave');

    rerender(<Skeleton.A11y animation="none" ariaLabel="Lädt Text" />);
  ...
  it('should handle multiple items correctly', () => {
    render(<Skeleton.A11y count={3} gap={10} ariaLabel="Lädt Liste" />);
  ...
  it('should handle loaded state correctly', () => {
    const { rerender } = render(
      <Skeleton.A11y isLoaded={false} ariaLabel="Lädt Text">
    ...
    rerender(
      <Skeleton.A11y isLoaded={true} ariaLabel="Lädt Text">
        <p>Geladener Inhalt</p>
      </Skeleton.A11y>
    );
  ...
  it('should handle hideWhenLoaded correctly', () => {
    const { rerender } = render(
      <Skeleton.A11y isLoaded={true} hideWhenLoaded={false} ariaLabel="Lädt Text">
        <p>Geladener Inhalt</p>
      </Skeleton.A11y>
    );
    ...
    rerender(<Skeleton.A11y isLoaded={true} hideWhenLoaded={true} ariaLabel="Lädt Text" />);
  ...
  it('should handle different live region politeness correctly', () => {
    const { rerender } = render(
      <Skeleton.A11y liveRegionPoliteness="assertive" ariaLabel="Lädt Text" />
    );

    let skeleton = screen.getByRole('status');
    expect(skeleton).toHaveAttribute('aria-live', 'assertive');

    rerender(<Skeleton.A11y liveRegionPoliteness="off" ariaLabel="Lädt Text" />);
  ...
  it('should handle aria-relevant correctly', () => {
    render(<Skeleton.A11y relevant="additions" ariaLabel="Lädt Text" />);
  ...
  it('should handle non-busy state correctly', () => {
    render(<Skeleton.A11y busy={false} ariaLabel="Lädt Text" />);
