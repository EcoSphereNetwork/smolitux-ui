...
const meta: Meta<typeof Modal.A11y> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Eine barrierefreie Version der Modal-Komponente mit verbesserten ARIA-Attributen und Screenreader-Unterstuetzung.'
      }
    }
  },
  argTypes: {
...
export const AlertDialog: Story = {
  args: {
    isOpen: true,
    title: 'Warnung',
    children: <p>Möchten Sie diese Aktion wirklich durchführen? Diese Aktion kann nicht rückgängig gemacht werden.</p>,
    isAlertDialog: true,
    ...
    return (
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => { setSize('xs'); setIsOpen(true); }}>Extra Small</Button>
...
export const DifferentPositions: Story = {
  render: () => {
    ...
    return (
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => { setPosition('center'); setIsOpen(true); }}>Center</Button>
          <Button onClick={() => { setPosition('top'); setIsOpen(true); }}>Top</Button>
          <Button onClick={() => { setPosition('bottom'); setIsOpen(true); }}>Bottom</Button>
          <Button onClick={() => { setPosition('left'); setIsOpen(true); }}>Left</Button>
          <Button onClick={() => { setPosition('right'); setIsOpen(true); }}>Right</Button>
        </div>
