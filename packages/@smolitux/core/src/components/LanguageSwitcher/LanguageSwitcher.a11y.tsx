...
export const LanguageSwitcherA11y: React.FC<LanguageSwitcherProps> = ({
  ...
  const handleButtonKeyDown = (event: KeyboardEvent<HTMLButtonElement>, locale: Locale, index: number) => {
    if (!keyboardNavigation || disabled) return;
      ...
      case 'dropdown':
        return (
          <div 
            ...
            aria-describedby={ariaDescribedby || (description ? 'language-switcher-description' : undefined)}
          ...
          >
            <div className="language-switcher-current">
              ...
              {showName && (
                <span className="language-name">{LOCALE_NAMES[currentLocale]}</span>
              )}
              {showCode && (
              ...
              >
                {availableLocales.map((locale, index) => (
                  ...
                  >
                    ...
                    {showName && (
                      <span className="language-name">{LOCALE_NAMES[locale]}</span>
                    )}
                    {showCode && (
