...
describe('FileUpload Accessibility', () => {
  ...
  describe('FileUpload.A11y Component', () => {
    ...
    it('should announce file selection to screen readers', () => {
      const { rerender } = render(
        <FileUpload.A11y label="Upload Files" accept="image/*,application/pdf" />
      );
      ...
      rerender(
        <FileUpload.A11y label="Upload Files" accept="image/*,application/pdf" value={[fileInfo]} />
      );
  ...
  it('should not have accessibility violations in basic state', async () => {
    const { violations } = await a11y.testA11y(<FileUpload label="Upload Files" />);
  ...
  it('should have accessible progress indicator', async () => {
    ...
    const progressContainer = screen
      .getByText('test.txt')
      .parentElement?.querySelector('.bg-gray-200');
