  ...
  (
    ...
    const variantClasses = {
      outlined: {
        ...
        active:
          'border border-primary-500 dark:border-primary-400 bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400',
        disabled:
          'border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-600 cursor-not-allowed',
      },
      filled: {
        default:
    ...
    const formatPageTemplate = (template: string, page: number, total: number) => {
      return template.replace('{page}', page.toString()).replace('{total}', total.toString());
    };
    ...
    return (
      <>
        ...
        <div
          ...
          {...rest}
        >
          <nav id={navId} aria-label={labels.pagination} aria-controls={pageListId}>
            <ul id={pageListId} className="flex items-center space-x-1" role="list">
              {/* Erste Seite */}
              {showFirstLast && (
                <li>
                  <button
                ...
                return (
                  <li key={pageNumber}>
                    <button
                      ...
                      aria-label={formatPageTemplate(
                        labels.pageTemplate || 'Seite {page} von {total}',
                        pageNumber,
                        pageCount
                      )}
