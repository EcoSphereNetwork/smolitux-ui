...
describe('CheckboxA11y', () => {
  ...
  test('connects helper text with checkbox using aria-describedby', () => {
    render(<CheckboxA11y label="Newsletter" helperText="You can unsubscribe at any time" />);
  ...
  test('connects error message with checkbox using aria-describedby and aria-invalid', () => {
    render(<CheckboxA11y label="Accept Terms" error="You must accept the terms" isInvalid />);
  ...
  test('connects success message with checkbox using aria-describedby', () => {
    render(<CheckboxA11y label="Email Preferences" successMessage="Preferences saved" isSuccess />);
  ...
  test('makes hidden label accessible for screen readers', () => {
    render(<CheckboxA11y label="Hidden Label" hideLabel />);
  ...
  test('makes description accessible for screen readers', () => {
    render(
      <CheckboxA11y label="Described Checkbox" description="This is a detailed description" />
    );
  ...
  test('handles keyboard interaction', async () => {
    ...
    render(<CheckboxA11y label="Keyboard Checkbox" onChange={handleChange} />);
  ...
  test('handles click events', async () => {
    const handleChange = jest.fn();
    render(<CheckboxA11y label="Clickable Checkbox" onChange={handleChange} />);
  ...
  test('does not update state when readonly', () => {
    render(<CheckboxA11y label="Readonly Checkbox" isReadOnly data-testid="readonly-checkbox" />);
  ...
  test('does not call onChange when disabled', async () => {
    const handleChange = jest.fn();
    render(<CheckboxA11y label="Disabled Checkbox" isDisabled onChange={handleChange} />);
  ...
  test('supports different sizes', () => {
    const { rerender } = render(<CheckboxA11y label="Small Checkbox" size="sm" />);

    let checkbox = screen.getByRole('checkbox');
    expect(checkbox).toHaveClass('h-4');
    expect(checkbox).toHaveClass('w-4');

    rerender(<CheckboxA11y label="Large Checkbox" size="lg" />);
  ...
  test('supports different variants', () => {
    const { rerender } = render(<CheckboxA11y label="Solid Checkbox" variant="solid" />);

    let checkbox = screen.getByRole('checkbox');
    expect(checkbox).toHaveClass('bg-white');
    expect(checkbox).toHaveClass('border-2');

    rerender(<CheckboxA11y label="Minimal Checkbox" variant="minimal" />);
  ...
  test('supports different color schemes', () => {
    const { rerender } = render(
      <CheckboxA11y label="Primary Checkbox" colorScheme="primary" checked />
    );

    let checkbox = screen.getByRole('checkbox');
    expect(checkbox).toHaveClass('checked:bg-primary-500');

    rerender(<CheckboxA11y label="Danger Checkbox" colorScheme="danger" checked />);
  ...
  test('supports different label positions', () => {
    const { rerender } = render(<CheckboxA11y label="Right Label" labelPosition="right" />);

    let container = screen.getByTestId('checkbox-container');
    expect(container).toHaveClass('flex-row');

    rerender(<CheckboxA11y label="Left Label" labelPosition="left" />);
  ...
  test('supports custom test ID', () => {
    render(<CheckboxA11y label="Custom TestID" testId="custom-checkbox" />);
  ...
  test('supports busy state', () => {
    render(<CheckboxA11y label="Busy Checkbox" busy />);
  ...
  test('supports loading state', () => {
    render(<CheckboxA11y label="Loading Checkbox" isLoading />);
