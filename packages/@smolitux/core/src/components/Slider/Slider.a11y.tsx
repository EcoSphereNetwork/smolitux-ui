...
export const SliderA11y = forwardRef<HTMLInputElement, SliderProps>(
  (
    ...
    const [currentValue2, setCurrentValue2] = useState(
      value2 !== undefined ? value2 : defaultValue2 || max
    );
    ...
    const getValueFromPosition = (position: number) => {
      ...
      if (orientation === 'horizontal') {
        percent = inverted ? (trackLength - position) / trackLength : position / trackLength;
      } else {
        percent = inverted ? position / trackLength : (trackLength - position) / trackLength;
      }
    ...
    const handleMouseMove = (event: React.MouseEvent<HTMLDivElement>, isSecondThumb = false) => {
      ...
      setAnnounceMessage(
        `${isSecondThumb ? 'Zweiter Wert' : 'Wert'}: ${formatValue(isSecondThumb ? newValue : newValue)}`
      );
    };
    ...
    const handleMouseMoveDocument = (event: MouseEvent) => {
      ...
      // Ankündige für Screenreader
      setAnnounceMessage(
        `${isDragging2 ? 'Zweiter Wert' : 'Wert'}: ${formatValue(isDragging2 ? newValue : newValue)}`
      );
    };
    ...
    const updateTooltipPositionTouch = (
    ...
    const renderValue = () => {
      ...
      const valueText = isRange ? `${formattedValue} - ${formattedValue2}` : formattedValue;
      ...
      return (
        <div
          className={`text-sm text-gray-700 dark:text-gray-300 ${positionClasses[valuePosition]}`}
        >
          {valueText}
        </div>
      );
    ...
    const renderTooltip = () => {
      ...
      const tooltipContent = typeof tooltip === 'object' ? tooltip : formattedValue;
          ...
          {marksList.map((mark, index) => {
            ...
            return (
              ...
              >
                ...
                ></div>
                {mark.label && (
                  <div className="mt-1 text-xs text-gray-600 dark:text-gray-400">{mark.label}</div>
                )}
              </div>
            );
          })}
