...
export const Slider = forwardRef<HTMLInputElement, SliderProps>(({
  ...
  const [internalValue2, setInternalValue2] = useState(value2 !== undefined ? value2 : defaultValue2);
  ...
  const styleClasses = {
    ...
    fluent: {
      ...
      filledTrack: 'bg-primary-500 dark:bg-primary-400 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80',
      thumb: 'bg-white shadow-md'
    },
    flat: {
    ...
    '3d': {
      ...
      filledTrack: 'bg-gradient-to-b from-primary-500 to-primary-600 dark:from-primary-400 dark:to-primary-500 shadow-inner',
    ...
    neon: {
      ...
      filledTrack: 'bg-primary-500 dark:bg-primary-400 shadow-[0_0_10px_rgba(59,130,246,0.5)] dark:shadow-[0_0_10px_rgba(96,165,250,0.5)]',
      thumb: 'bg-primary-500 dark:bg-primary-400 shadow-[0_0_10px_rgba(59,130,246,0.5)] dark:shadow-[0_0_10px_rgba(96,165,250,0.5)]'
    },
    glass: {
      ...
      filledTrack: 'bg-primary-500 bg-opacity-20 dark:bg-primary-400 dark:bg-opacity-20 backdrop-blur-md border border-primary-500 border-opacity-30 dark:border-primary-400',
    ...
    neumorphic: {
      ...
      filledTrack: 'bg-gray-200 dark:bg-gray-800 shadow-[inset_3px_3px_6px_rgba(0,0,0,0.1),inset_-3px_-3px_6px_rgba(255,255,255,0.7)] dark:shadow-[inset_3px_3px_6px_rgba(0,0,0,0.2),inset_-3px_-3px_6px_rgba(255,255,255,0.1)]',
    ...
    skeuomorphic: {
      ...
      filledTrack: 'bg-gradient-to-b from-primary-400 to-primary-500 dark:from-primary-700 dark:to-primary-800 border border-primary-600 dark:border-primary-900 shadow-inner',
    ...
    retro: {
      ...
      filledTrack: 'bg-primary-500 dark:bg-primary-700 border-2 border-primary-700 dark:border-primary-500',
  ...
  }, [isDragging, isDragging2, internalValue, internalValue2, isRange, onChange, onChange2, tooltip]);
  ...
  }, [isDragging, isDragging2, internalValue, internalValue2, onChangeEnd, onChangeEnd2, tooltip]);
  ...
  const generateMarks = () => {
    ...
    const marksToRender = marks || Array.from({ length: Math.floor((max - min) / step) + 1 }, (_, i) => ({
      value: min + i * step
    }));
  ...
  const generateScale = () => {
    ...
    const scaleValues = Array.from({ length: scaleSteps + 1 }, (_, i) => min + (i * (max - min)) / scaleSteps);
  ...
  const generateHighlightedRanges = () => {
    ...
    const ranges = highlightRanges || [{ range: highlightRange as [number, number], color: highlightColor }];
    ...
    return ranges.map((rangeObj, index) => {
      ...
      const rangeStyle: React.CSSProperties = isVertical || orientation === 'vertical'
        ? {
            position: 'absolute',
  ...
  const renderTooltip = () => {
    ...
    const tooltipContent = typeof tooltip === 'function'
  ...
  const renderTooltip2 = () => {
    ...
    const tooltipContent = typeof tooltip === 'function'
      ? tooltip(internalValue2)
      : formatValue(internalValue2);
  ...
  const renderSlider = () => {
    ...
    const thumb1Styles: React.CSSProperties = isVertical || orientation === 'vertical'
      ? { bottom: `${percentage1}%` }
      : { left: `${percentage1}%` };
    
    const thumb2Styles: React.CSSProperties = isVertical || orientation === 'vertical'
      ? { bottom: `${percentage2}%` }
      : { left: `${percentage2}%` };
