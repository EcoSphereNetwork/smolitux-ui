...
export const TimePickerA11y = forwardRef<HTMLInputElement, TimePickerProps>(({
  ...
  const [focusedSection, setFocusedSection] = useState<'hours' | 'minutes' | 'seconds' | 'period' | null>(null);
  ...
  function isValidTime(time: TimeValue): boolean {
    ...
    if (minTime) {
      ...
      const minTotalSeconds = minTimeValue.hours * 3600 + minTimeValue.minutes * 60 + (minTimeValue.seconds || 0);
    ...
    if (maxTime) {
      ...
      const maxTotalSeconds = maxTimeValue.hours * 3600 + maxTimeValue.minutes * 60 + (maxTimeValue.seconds || 0);
      const timeTotalSeconds = hours * 3600 + minutes * 60 + (seconds || 0);
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        ...
        if (focusedSection === 'hours') {
          ...
          const currentIndex = hoursList.indexOf(format === '12h' ? convertTo12Hour(selectedTime.hours).hours : selectedTime.hours);
  ...
  const handleHoursFocus = () => {
    setFocusedSection('hours');
    setFocusedValue(format === '12h' ? convertTo12Hour(selectedTime.hours).hours : selectedTime.hours);
  };
  ...
  const renderPopup = () => {
      ...
      >
        <div className="flex space-x-4">
          ...
          <div 
            ...
            aria-activedescendant={focusedSection === 'hours' ? `${hoursListId}-${focusedValue}` : undefined}
            onFocus={handleHoursFocus}
          >
            <div id={`${hoursListId}-label`} className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Stunden
            </div>
          ...
          <div 
            ...
            aria-activedescendant={focusedSection === 'minutes' ? `${minutesListId}-${focusedValue}` : undefined}
            onFocus={handleMinutesFocus}
          >
            <div id={`${minutesListId}-label`} className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Minuten
            </div>
            <div className="h-48 overflow-y-auto">
          ...
          {!hideSeconds && (
            <div 
              ...
              aria-activedescendant={focusedSection === 'seconds' ? `${secondsListId}-${focusedValue}` : undefined}
              onFocus={handleSecondsFocus}
            >
              <div id={`${secondsListId}-label`} className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Sekunden
              </div>
              <div className="h-48 overflow-y-auto">
          ...
          {format === '12h' && (
            <div 
              ...
              aria-activedescendant={focusedSection === 'period' ? `${periodListId}-${focusedValue}` : undefined}
              onFocus={handlePeriodFocus}
            >
              <div id={`${periodListId}-label`} className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Periode
              </div>
              <div>
    ...
    return portalTarget
      ? createPortal(popupContent, portalTarget)
      : popupContent;
  };
  ...
  return (
    <div className={`relative ${fullWidth ? 'w-full' : ''} ${containerClassName} ${className}`}>
      ...
      <div id={`${timePickerId}-instructions`} className="sr-only">
        Drücken Sie Enter oder die Leertaste, um den Zeitauswahldialog zu öffnen. Verwenden Sie die Pfeiltasten, um die Zeit zu ändern.
      </div>
    </div>
  );
});
