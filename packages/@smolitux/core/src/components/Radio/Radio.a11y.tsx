import React, { forwardRef, useRef, useEffect, useState, useContext, useMemo, useCallback, useId } from 'react';
...
export const RadioA11y = forwardRef<HTMLInputElement, RadioProps>(({
  ...
  const _id = id || radioGroup?.getRadioId?.(value as string) || formControl.id || `radio-${uniqueId}`;
      ...
      <div className="relative flex items-center">
        ...
        {announceChecked && (
          <span className="sr-only">
            {_checked ? checkedStateText : uncheckedStateText}
          </span>
        )}
      </div>
    );
