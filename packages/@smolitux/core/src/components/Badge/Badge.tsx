...
export const Badge = forwardRef<HTMLSpanElement, BadgeProps>(
  (
    ...
    const ariaProps = {
      ...
      ...(isDot ? { 'data-dot': 'true', 'aria-hidden': 'true' as any } : {}),
      ...htmlProps,
    };

    if (isDot) {
      return <span ref={ref} className={badgeClasses} {...ariaProps} />;
    }

    return (
      <span ref={ref} className={badgeClasses} {...ariaProps}>
        {icon && (
          <span className="mr-1" aria-hidden={'true' as any}>
            {icon}
          </span>
        )}
        {displayContent}
