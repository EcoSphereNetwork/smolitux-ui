...
describe('Select', () => {
  ...
  it('has proper ARIA attributes', () => {
    render(<Select options={options} label="Test Label" required={true} error="Error message" />);
  ...
  it('allows multiple selections', () => {
    const handleChange = jest.fn();
    render(<Select isMulti onChange={handleChange} options={options} />);

    const select = screen.getByTestId('select-element');
    fireEvent.change(select, {
      target: {
        selectedOptions: [{ value: 'option1' }, { value: 'option3' }],
      },
    });
