...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  (
    ...
    // Hover-Klassen
    const hoverClasses =
      hoverable && !disabled && !readOnly ? 'hover:border-gray-400 dark:hover:border-gray-500' : '';

    // Transitions-Klassen
    const transitionClasses = transition ? 'transition duration-150 ease-in-out' : '';
    ...
    const groupedOptions =
      groupOptions && options && Array.isArray(options)
        ? options.reduce(
            (acc, option) => {
    ...
    const renderOption = (option: SelectOption) => (
      <option
        ...
        data-description={
          showOptionDescription && option.description ? option.description : undefined
        }
